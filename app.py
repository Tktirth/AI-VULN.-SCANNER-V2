...
PHASE_LABELS = {
    ...
    "idor":       "🔓 IDOR Fuzzing",
    "ip_leakage": "🔍 IP Leakage Detection",
...
with st.sidebar:
    ...
    scan_recon      = st.checkbox("🌐 IP Recon (Phase 0)",    value=True)
    scan_ip_leakage = st.checkbox("🔍 IP Leakage Detection",  value=True)
  ...
  <div>
    <div style="display:flex;align-items:center;gap:0.75rem;">
      ...
      <div>
        ...
        <p class="v2-subtitle">
          Auth · IP Recon · CDN Bypass · XSS · Stored XSS · SQLi · IDOR · Headers · Redirects · Dirs &nbsp;·&nbsp;
          IP Leakage · CVSS Scoring · CWE References
        </p>
      </div>
    </div>
  </div>
    ...
    with st.spinner("Scanning..."):
        try:
            engine = ScannerEngineV2(
                ...
                scan_ip_leakage=scan_ip_leakage,
                max_pages=max_pages,
            ...
            st.session_state.recon_data       = summary.get("recon_data", {})
            st.session_state.report_json = report_to_json_string(
                target_url, vulns, summary,
                recon_data=summary.get("recon_data"),
            )
...
if st.session_state.done:
    ...
    # Recon panel (IP Intelligence)
    recon = st.session_state.get("recon_data", {})
    if recon and recon.get("target_domain"):
        st.markdown('<div class="section-label">🔎 IP Intelligence</div>', unsafe_allow_html=True)

        # Row 1 — Target Intelligence pills
        ip_val = recon.get('primary_ip') or '—'
        ptr_val = recon.get('ptr_record') or '—'
        asn_num = recon.get('asn_number') or ''
        asn_org = recon.get('asn_org') or ''
        asn_val = f"{asn_num} · {asn_org}" if asn_num else '—'
        country = recon.get('hosting_country') or '—'

        st.markdown(f"""
        <div style="display:flex;gap:0.6rem;flex-wrap:wrap;margin-bottom:0.8rem;">
          <div style="background:var(--bg3);border:1px solid var(--border2);border-radius:6px;padding:0.4rem 0.8rem;font-family:var(--mono);font-size:0.78rem;">
            <span style="color:var(--text-dim);font-size:0.65rem;text-transform:uppercase;letter-spacing:0.1em;">IP</span><br/>
            <span style="color:var(--green);">{ip_val}</span>
          </div>
          <div style="background:var(--bg3);border:1px solid var(--border2);border-radius:6px;padding:0.4rem 0.8rem;font-family:var(--mono);font-size:0.78rem;">
            <span style="color:var(--text-dim);font-size:0.65rem;text-transform:uppercase;letter-spacing:0.1em;">PTR</span><br/>
            <span style="color:var(--text);">{ptr_val}</span>
          </div>
          <div style="background:var(--bg3);border:1px solid var(--border2);border-radius:6px;padding:0.4rem 0.8rem;font-family:var(--mono);font-size:0.78rem;">
            <span style="color:var(--text-dim);font-size:0.65rem;text-transform:uppercase;letter-spacing:0.1em;">ASN</span><br/>
            <span style="color:var(--text);">{asn_val}</span>
          </div>
          <div style="background:var(--bg3);border:1px solid var(--border2);border-radius:6px;padding:0.4rem 0.8rem;font-family:var(--mono);font-size:0.78rem;">
            <span style="color:var(--text-dim);font-size:0.65rem;text-transform:uppercase;letter-spacing:0.1em;">COUNTRY</span><br/>
        ...
        # Row 2 — CDN Status
        cdn_html_parts = []
        if recon.get('is_behind_cdn'):
            provider = recon.get('cdn_provider', 'Unknown CDN')
            cdn_html_parts.append(
                f'<span style="background:rgba(234,179,8,0.12);border:1px solid #eab308;color:#eab308;'
                f'padding:0.25rem 0.7rem;border-radius:4px;font-family:var(--mono);font-size:0.75rem;font-weight:600;">'
                f'🛡 Behind {provider}</span>'
            )
            if recon.get('cdn_bypass_possible'):
                origin_ip = recon.get('most_likely_origin_ip', '???')
                method = recon.get('origin_discovery_method', '')
                cdn_html_parts.append(
                    f'<span style="background:rgba(244,63,94,0.12);border:1px solid #f43f5e;color:#f43f5e;'
                    f'padding:0.25rem 0.7rem;border-radius:4px;font-family:var(--mono);font-size:0.75rem;font-weight:600;"'
                    f' title="Discovered via {method}. Scanning this IP directly may bypass CDN/WAF protections.">'
                    f'⚠ Origin IP: {origin_ip}</span>'
                )
            else:
                cdn_html_parts.append(
                    '<span style="background:rgba(34,211,165,0.1);border:1px solid #22d3a5;color:#22d3a5;'
                    'padding:0.25rem 0.7rem;border-radius:4px;font-family:var(--mono);font-size:0.75rem;font-weight:600;">'
                    '✓ No Origin Leaked</span>'
                )
        else:
            cdn_html_parts.append(
                '<span style="background:rgba(34,211,165,0.1);border:1px solid #22d3a5;color:#22d3a5;'
        ...
        # Internal IP leakage count
        leak_count = recon.get('internal_ips_found', 0)
        if leak_count > 0:
            cdn_html_parts.append(
                f'<span style="background:rgba(249,115,22,0.12);border:1px solid #f97316;color:#f97316;'
                f'padding:0.25rem 0.7rem;border-radius:4px;font-family:var(--mono);font-size:0.75rem;font-weight:600;">'
                f'🔍 {leak_count} Internal IP(s) Leaked</span>'
            )

        st.markdown(
            f'<div style="display:flex;gap:0.6rem;flex-wrap:wrap;margin-bottom:1rem;">'
            f'{" ".join(cdn_html_parts)}</div>',
            unsafe_allow_html=True,
        )

        # Expandable detailed recon view
        with st.expander("📋 Full Recon Details"):
            recon_cols = st.columns(2)
            with recon_cols[0]:
                st.markdown("**Resolved IPs (v4)**")
                for ip in recon.get('resolved_ips', []):
                    st.code(ip, language=None)
                if recon.get('ipv6_addresses'):
                    st.markdown("**IPv6 Addresses**")
                    for ip in recon.get('ipv6_addresses', []):
                        st.code(ip, language=None)
            with recon_cols[1]:
                if recon.get('ssl_cert_domains'):
                    st.markdown("**SSL Certificate Domains (SANs)**")
                    for d in recon.get('ssl_cert_domains', [])[:20]:
                        st.text(d)
                    total_sans = len(recon.get('ssl_cert_domains', []))
                    if total_sans > 20:
                        st.caption(f"... and {total_sans - 20} more")

            if recon.get('origin_ip_candidates'):
                st.markdown("**Origin IP Candidates**")
                cand_df = pd.DataFrame(recon['origin_ip_candidates'])
                st.dataframe(cand_df, use_container_width=True, hide_index=True)

            if recon.get('mx_records'):
                st.markdown("**MX Records**")
                mx_df = pd.DataFrame(recon['mx_records'])
                st.dataframe(mx_df, use_container_width=True, hide_index=True)

            if recon.get('spf_ip_ranges'):
                st.markdown("**SPF IP Ranges**")
                for r in recon.get('spf_ip_ranges', []):
                    st.code(r, language=None)

            if recon.get('ns_records'):
                st.markdown("**NS Records**")
                ns_df = pd.DataFrame(recon['ns_records'])
    ...
    <div class="landing">
      ...
      AUTH · IP RECON · CDN BYPASS · XSS · STORED XSS · SQLi · IDOR · HEADERS · REDIRECT · DIRS · IP LEAKAGE · CVSS · CWE
      </div>
    </div>
    """, unsafe_allow_html=True)
